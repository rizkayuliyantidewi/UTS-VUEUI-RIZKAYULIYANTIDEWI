...
export default {
  setup() {
    ...
    onMounted(() => {
      axios.get('http://localhost:8000/api/kontraks/${route.params.id}')
        .then(response  => {
            console.log(response)


          kontrak.id = response.data.data.id
          kontrak.nama_mahasiswa = response.data.data.nama_mahasiswa
          kontrak.semester = response.data.data.semester
    ...
    function update() {
      ...
      axios.put('http://localhost:8000/api/kontraks/${route.params.id}',{
          id: id,
